...
import {
  Plus, Trash2, X,
  Filter, XCircle, FileText, GripVertical, Columns3,
} from 'lucide-react';
...
const METHOD_COLORS: Record<string, string> = {
  '天河': 'bg-sky-100 text-sky-800 border-sky-300',
  '番禺': 'bg-teal-100 text-teal-800 border-teal-300',
  '调拨天河': 'bg-indigo-100 text-indigo-800 border-indigo-300',
  '调拨番禺': 'bg-violet-100 text-violet-800 border-violet-300',
  '直发': 'bg-orange-100 text-orange-800 border-orange-300',
};

const ORDER_FORM_COLORS: Record<string, string> = {
  '常规': 'bg-emerald-100 text-emerald-800 border-emerald-300',
  '线下': 'bg-amber-100 text-amber-800 border-amber-300',
  '后补单': 'bg-rose-100 text-rose-800 border-rose-300',
  '线上': 'bg-cyan-100 text-cyan-800 border-cyan-300',
};
...
function ProcurementTableSection() {
  ...
  // ─── dynamic column order & custom columns ─────────────────────
  const COLUMN_CONFIG_KEY = '__app_reagent_column_config';
  interface ICustomColumn { id: string; key: string; label: string; }

  const [visibleColumnOrder, setVisibleColumnOrder] = useState<string[]>(() => {
    try {
      const raw = scopedStorage.getItem(COLUMN_CONFIG_KEY);
      if (raw) {
        const cfg = JSON.parse(raw) as { order?: string[]; custom?: ICustomColumn[] };
        if (Array.isArray(cfg.order) && cfg.order.length > 0) return cfg.order;
      }
    } catch { /* */ }
    return [...DEFAULT_FIELD_ORDER];
  });
  const [customColumns, setCustomColumns] = useState<ICustomColumn[]>(() => {
    try {
      const raw = scopedStorage.getItem(COLUMN_CONFIG_KEY);
      if (raw) {
        const cfg = JSON.parse(raw) as { custom?: ICustomColumn[] };
        return cfg.custom ?? [];
      }
    } catch { /* */ }
    return [];
  ...
  const dragSourceFieldRef = useRef<string | null>(null);
  const [dropTargetField, setDropTargetField] = useState<string | null>(null);

  const saveColumnConfig = useCallback((order: string[], custom: ICustomColumn[]) => {
    scopedStorage.setItem(COLUMN_CONFIG_KEY, JSON.stringify({ order, custom }));
  }, []);
  const isCustomCol = useCallback((key: string) => customColumns.some(c => c.key === key), [customColumns]);
  const getColLabel = useCallback((key: string) => {
    if (FIELD_LABELS[key]) return FIELD_LABELS[key];
    const c = customColumns.find(col => col.key === key);
    return c?.label ?? key;
  }, [customColumns]);
  const reorderColumn = useCallback((fromKey: string, toKey: string) => {
    setVisibleColumnOrder(prev => {
      const next = [...prev];
      ...
      saveColumnConfig(next, customColumns);
      return next;
    });
  }, [customColumns, saveColumnConfig]);
  const removeColumn = useCallback((key: string) => {
    ...
    setCustomColumns(prev => {
      ...
      saveColumnConfig(visibleColumnOrder.filter(k => k !== key), next);
      return next;
    });
  }, [customColumns, visibleColumnOrder, saveColumnConfig]);
  const addColumn = useCallback(() => {
    const label = newColLabel.trim();
    const key = newColKey.trim() || `custom_${Date.now()}`;
    if (!label) { toast.error('请输入列名称'); return; }
    if (visibleColumnOrder.includes(key)) { toast.error('该列标识已存在'); return; }
    const col: ICustomColumn = { id: String(Date.now()), key, label };
    const nextOrder = [...visibleColumnOrder, key];
    const nextCustom = [...customColumns, col];
    setVisibleColumnOrder(nextOrder);
    setCustomColumns(nextCustom);
    saveColumnConfig(nextOrder, nextCustom);
    setNewColLabel(''); setNewColKey('');
    toast.success(`已添加列「${label}」`);
  }, [newColLabel, newColKey, visibleColumnOrder, customColumns, saveColumnConfig]);
  ...
  const handleEditValueChange = useCallback((value: string, field: keyof IOrder) => {
    setEditValue(value);
    // 文本字段：输入时弹出同列记忆建议（合并历史记忆 + 当前表格已有数据）
    ...
    if (textFields.includes(field) && value.trim().length >= 1) {
      ...
      // 1) 历史记忆匹配
      const historyMatches = suggest(field, value);
      // 2) 从当前表格数据中提取同列已有值
      const existingValues = Array.from(
        new Set(
          orders
            .map(o => String(o[field] ?? '').trim())
            .filter(v => v && v.toLowerCase().includes(q))
        )
      );
      // 3) 合并去重，历史记忆优先
      const merged = [...new Set([...historyMatches, ...existingValues])].slice(0, 8);
  ...
  const onCopy = useCallback((e: React.ClipboardEvent) => {
    ...
    for (let r = minRow; r <= maxRow; r++) {
      ...
      for (let c = minCol; c <= maxCol; c++) {
        if (c < 0 || c >= visibleColumnOrder.length) continue;
        cells.push(String((filteredOrders[r] as unknown as Record<string, unknown>)[visibleColumnOrder[c]] ?? ''));
      }
    ...
    if (selection) {
      ...
      for (let r = 0; r < pasteRows; r++) {
        ...
        for (let c = 0; c < pasteCols; c++) {
          ...
          if (targetColIdx >= visibleColumnOrder.length) break;
          const field = visibleColumnOrder[targetColIdx];
          const val = cells[c] ?? '';
          if (field === 'listPrice' || field === 'quantity' || field === 'unitPrice' || field === 'totalPrice') {
  ...
  const renderCellValue = (order: IOrder, field: keyof IOrder) => {
    if (isCustomCol(field as string)) {
      return String((order as unknown as Record<string, unknown>)[field as string] ?? '');
    }
    ...
    if (field === 'procurementMethod') {
      const v = String(val ?? '');
      return v ? <Badge className={`text-xs font-semibold border ${METHOD_COLORS[v] || 'bg-muted text-muted-foreground border-border'}`}>{v}</Badge> : '';
    }
    if (field === 'orderForm') {
      const v = String(val ?? '');
      return v ? <Badge className={`text-xs font-semibold border ${ORDER_FORM_COLORS[v] || 'bg-muted text-muted-foreground border-border'}`}>{v}</Badge> : '';
    }
  ...
  const renderEditableCell = (order: IOrder, field: keyof IOrder) => {
    if (isEditing(order.id, field)) {
      if (isCustomCol(field as string)) {
        return (
          <Input
            className="h-8 text-xs font-semibold text-center w-full"
            value={editValue}
      ...
      if (field === 'status') {
        return (
          <Select value={editValue} onValueChange={(val) => { changeStatus(order.id, val as IOrder['status']); setEditingCell(null); }}>
        ...
        return (
          <Select value={editValue} onValueChange={(val) => {
            ...
            const next = orders.map(o => {
              ...
              return { ...o, orderForm: val as IOrder['orderForm'], updatedAt: new Date().toISOString() };
            });
            persistOrders(next);
          }}>
            <SelectTrigger className="h-8 w-full text-xs font-semibold">
      ...
      if (field === 'procurementMethod') {
        return (
          <Select value={editValue} onValueChange={(val) => {
            ...
            const next = orders.map(o => {
              if (o.id !== order.id) return o;
              return { ...o, procurementMethod: val, updatedAt: new Date().toISOString() };
            });
            persistOrders(next);
          }}>
            <SelectTrigger className="h-8 w-full text-xs font-semibold">
            ...
            onKeyDown={e => {
              if (suggestions.length > 0) {
                ...
                if (e.key === 'Tab') {
                  e.preventDefault();
                  commitEdit();
                  const idx = visibleColumnOrder.indexOf(field as string);
                  if (idx < visibleColumnOrder.length - 1) {
                    const nf = visibleColumnOrder[idx + 1] as keyof IOrder;
                    startEdit(order.id, nf, (order as unknown as Record<string, string | number>)[nf]);
                  }
                  return;
                }
    ...
    >
      ...
      <div className="flex flex-col gap-2 px-4 py-3 border-b border-border shrink-0">
        ...
        <div className="flex items-center flex-wrap gap-2">
          ...
          <Button size="sm" variant="outline" onClick={() => setColumnMgrOpen(true)}>
            <Columns3 className="size-3.5" />
            <span className="ml-1">列管理</span>
          </Button>
          <Button
            size="sm"
        ...
        {filterOpen && (
          <div className="flex flex-wrap items-center gap-2 p-2 bg-muted/30 rounded-md">
            {visibleColumnOrder.map(field => (
              <div key={field} className="flex items-center gap-1">
                <span className="text-xs text-muted-foreground whitespace-nowrap">{getColLabel(field)}:</span>
                <Input
        ...
        <table className="w-full border-collapse table-fixed min-w-[1400px]">
          <thead className="sticky top-0 z-30">
            <tr>
              ...
              {visibleColumnOrder.map(field => (
                <th
                  key={field}
                  className={`relative border-b-2 border-border bg-muted/90 backdrop-blur-sm px-2 py-3 text-xs font-bold text-foreground text-center select-none cursor-grab active:cursor-grabbing ${dropTargetField === field ? 'ring-2 ring-primary ring-inset' : ''}`}
                  style={{ width: colWidths[field] || 120, minWidth: 60 }}
                  draggable
                  onDragStart={() => { dragSourceFieldRef.current = field; }}
                  onDragOver={e => { e.preventDefault(); setDropTargetField(field); }}
                  onDragLeave={() => { setDropTargetField(prev => prev === field ? null : prev); }}
                  onDrop={() => { if (dragSourceFieldRef.current && dragSourceFieldRef.current !== field) reorderColumn(dragSourceFieldRef.current, field); dragSourceFieldRef.current = null; setDropTargetField(null); }}
                  onDragEnd={() => { dragSourceFieldRef.current = null; setDropTargetField(null); }}
                >
                  <GripVertical className="size-3 inline-block mr-0.5 text-muted-foreground/50 cursor-grab" />
                  {getColLabel(field)}
                  <div
          ...
          <tbody>
            {filteredOrders.length === 0 && (
              <tr>
                <td colSpan={visibleColumnOrder.length + 2} className="py-16 text-center text-muted-foreground text-sm">
            ...
            {filteredOrders.map((order, rowIdx) => (
              <tr key={order.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                ...
                {visibleColumnOrder.map((field, colIdx) => (
                  <td
                    key={field}
                  ...
                  >
                    {renderEditableCell(order, field as keyof IOrder)}
                  </td>
                ))}
      ...
      <Dialog open={columnMgrOpen} onOpenChange={setColumnMgrOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-base">列管理</DialogTitle>
            <DialogDescription>拖拽列名调整顺序，可添加自定义列或删除非默认列</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 max-h-[50vh] overflow-y-auto">
            {visibleColumnOrder.map(key => (
              <div key={key} className="flex items-center gap-2 p-2 rounded-md border border-border/50 bg-card">
                <GripVertical className="size-4 text-muted-foreground/40 shrink-0" />
                <span className="flex-1 text-sm font-medium">{getColLabel(key)}</span>
                {DEFAULT_FIELD_ORDER.includes(key) ? (
                  <Badge variant="outline" className="text-[10px]">默认</Badge>
                ) : (
                  <Button size="icon" variant="ghost" className="h-6 w-6 text-destructive" onClick={() => removeColumn(key)}>
          ...
          <div className="border-t border-border pt-3 space-y-2">
            <p className="text-xs font-semibold">添加自定义列</p>
            <div className="flex gap-2">
              <Input className="h-8 text-sm flex-1" placeholder="列名称" value={newColLabel} onChange={e => setNewColLabel(e.target.value)} />
              <Button size="sm" onClick={addColumn} disabled={!newColLabel.trim()}>
